  ...
  return (
    <div className="flex max-w-[75ch] mx-4 sm:mx-auto flex-col gap-4 empty:hidden first:pt-[3px] lesson">
      ...
      <p>
        ...
        changes that occur in a web application&#39;s UI by efficiently updating
        the DOM. Understanding how DOM reconciliation works can improve
        performance and user experience by minimizing unnecessary updates. Let’s
      ...
      <p>
        ...
        (e.g., paragraphs, divs, images) is a node in this tree. JavaScript can
        interact with this structure to create, delete, or modify elements
        dynamically.
      </p>
      <h2>
      ...
      <p>
        DOM reconciliation is the process of updating the UI in an efficient way
        when the underlying data changes. Instead of re-rendering the entire
        page when something changes, reconciliation optimizes updates to only
        modify the parts of the DOM that have actually changed.
      </p>
      <h2>
      ...
      <p>
        ...
        interacting with the document. Frequent updates can degrade performance,
        especially for complex applications. Reconciliation aims to:
      </p>
      <ul>
        <li>
          <strong>Minimize Repaints and Reflows</strong>: Changes in the DOM can
          trigger re-layouts (reflows) and repaints in the browser, which are
          costly operations.
        </li>
        <li>
          <strong>Optimize User Experience</strong>: By updating only the
          necessary parts of the DOM, applications remain smooth and responsive.
        </li>
      </ul>
      <h2>
      ...
      <ul>
        <li>
          <strong>Virtual DOM</strong>: A lightweight representation of the real
          DOM that lives in memory. It is like a copy of the real DOM.
        </li>
        <li>
      ...
      <p>
        ...
        update the real DOM. Instead, it updates the Virtual DOM first, compares
        the new Virtual DOM with the old one (called <strong>diffing</strong>),
        and then applies only the changes (called <strong>patching</strong>) to
        the real DOM.
      </p>
      <h2>
...
`}
        ...
        <li>
          ...
          <ul className="ml-4">
            <li>
              If a new item is added to the list, React updates the Virtual DOM.
            </li>
            <li>
...
`}
        ...
        <li>
          ...
          <ul className="ml-4">
            <li>
              React updates the real DOM by adding a new <code>&lt;li&gt;</code>{" "}
              element for <code>Item 4</code>.
            </li>
            <li>
              Only this part of the DOM is updated, rather than re-rendering the
              entire <code>&lt;ul&gt;</code> or <code>&lt;div&gt;</code>.
            </li>
          </ul>
        </li>
      </ol>
      ...
      <ul className="ml-4">
        <li>
          The diffing algorithm identifies what has changed between the previous
          and new versions of the Virtual DOM.
        </li>
        <li>It compares nodes one by one.</li>
        <li>
          If two nodes are of different types (e.g., an <code>&lt;h1&gt;</code>{" "}
          vs. a <code>&lt;p&gt;</code>), the entire subtree is re-rendered.
        </li>
        <li>
          If they are of the same type (e.g., <code>&lt;ul&gt;</code> vs.{" "}
          <code>&lt;ul&gt;</code>), it compares the attributes and children, and
...
`}
      ...
      <ul>
        ...
        <li>
          Using <strong>unique keys</strong> for lists and optimizing re-renders
          with hooks (like <code>React.memo</code>) can further enhance
          performance.
        </li>
      </ul>
      <p>
        This tutorial provides a basic understanding of how DOM reconciliation
        works, particularly in frameworks like React. It helps developers build
        more performant and responsive web applications. Happy coding!
      </p>
    </div>
  );
}
