  ...
  return (
    <div className="flex max-w-[75ch] mx-4 sm:mx-auto flex-col gap-4 empty:hidden first:pt-[3px] lesson">
      ...
      <p>
        ...
        where a server does not store any session information but relies on the
        validity of the token provided by the client.
      </p>
      <p>
        A typical JWT might look like this (yes, they're long and you'll have to
...
`}
      ...
      <ol>
        <li>
          ...
          the type of token (<code>JWT</code>) and the signing algorithm (e.g.,{" "}
          <code>HS256</code>).
        </li>
        <li>
        ...
        <li>
          <strong>Signature</strong>: Verifies that the sender of the JWT is who
          it says it is and ensures that the token has not been altered.
        </li>
      </ol>
      <p>Example JWT:</p>
      <CodeHighlight
...
`}
      ...
      <ul>
        ...
        <li>
          The client can verify the token using the <strong>secret</strong> (for{" "}
          <code>HS256</code>) or the <strong>public key</strong> (for{" "}
          <code>RS256</code>).
        </li>
      </ul>
      <p>
        This signature is what makes the token tamper-proof. If anyone tries to
        change the data in the payload, the signature will no longer match, and
        the token will be rejected during verification.
      </p>
      <hr />
      <h2>JWT Use Cases</h2>
      <ul>
        <li>
          <strong>Authentication</strong>: JWTs are commonly used in user login
          systems. Once a user logs in, the server issues a token to the client.
          The client sends this token with every subsequent request, and the
          server verifies the token to authenticate the user.
        </li>
        <li>
          <strong>Authorization</strong>: JWTs can contain user roles and
          permissions, making it easier for servers to determine what resources
          a user can access.
        </li>
        <li>
          <strong>Data Exchange</strong>: JWTs can be used to securely transfer
          data between parties, ensuring that the data has not been tampered
          with.
        </li>
      </ul>
      <hr />
      ...
      <p>
        Let&apos;s create a JWT using Node.js and the <code>jsonwebtoken</code>{" "}
        library.
      </p>
      <h3>
...
`}
      ...
      <ul>
        <li>
          <strong>Size</strong>: Because JWTs contain all their claims, they can
          become large, especially when encoded as a base64 string.
        </li>
        <li>
      ...
      <ul>
        ...
        <li>
          <strong>Do Not Store Sensitive Data</strong>: Avoid storing sensitive
          user data like passwords or personally identifiable information (PII)
          in the JWT payload.
        </li>
        <li>
      ...
      <p>
        ...
        communication between clients and servers, making them a popular choice
        for RESTful APIs and Single Page Applications (SPAs). While they offer
        many benefits, it&apos;s crucial to follow best practices to ensure the
        security and integrity of your tokens.
      </p>
      <p>
        With a solid understanding of JWTs, you can enhance the security of your
        applications and provide a seamless user experience.
      </p>
    </div>
  );
}
